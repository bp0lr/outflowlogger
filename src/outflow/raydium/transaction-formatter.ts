...
import { AccountMeta, CompiledInstruction, ConfirmedTransactionMeta, LoadedAddresses, Message, MessageCompiledInstruction, MessageV0, PublicKey, TransactionInstruction, VersionedMessage, VersionedTransactionResponse } from "@solana/web3.js";
...
export class TransactionFormatter {
  public formTransactionFromJson(data: any, time: number): VersionedTransactionResponse {
    ...
    const signatures = rawTx.transaction.signatures.map((s: Buffer) => base58.encode(s));
  ...
  private formTxnMessage(message: any): VersionedMessage {
    if (!message.versioned) {
      return new Message({
        ...
        recentBlockhash: base58.encode(Buffer.from(message.recentBlockhash, "base64")),
        accountKeys: message.accountKeys?.map((d: string) => Buffer.from(d, "base64")),
        instructions: message.instructions.map(({ data, programIdIndex, accounts }: { data: any; programIdIndex: any; accounts: any }) => ({
      ...
      return new MessageV0({
        ...
        recentBlockhash: base58.encode(Buffer.from(message.recentBlockhash, "base64")),
        staticAccountKeys: message.accountKeys.map((k: string) => new PublicKey(Buffer.from(k, "base64"))),
        compiledInstructions: message.instructions.map(({ programIdIndex, accounts, data }: { programIdIndex: any; accounts: any; data: any }) => ({
          programIdIndex: programIdIndex,
          accountKeyIndexes: Array.isArray(accounts) ? [...accounts] : [],
          data: Uint8Array.from(Buffer.from(data || "", "base64")),
        })),
        addressTableLookups:
          message.addressTableLookups?.map(({ accountKey, writableIndexes, readonlyIndexes }: { accountKey: any; writableIndexes: any; readonlyIndexes: any }) => ({
    ...
    return {
      ...
      loadedAddresses:
        meta.loadedWritableAddresses || meta.loadedReadonlyAddresses
          ? {
              writable: meta.loadedWritableAddresses?.map((address: string) => new PublicKey(Buffer.from(address, "base64"))) || [],
              readonly: meta.loadedReadonlyAddresses?.map((address: string) => new PublicKey(Buffer.from(address, "base64"))) || [],
            }
          : undefined,
      innerInstructions:
        meta.innerInstructions?.map((i: { index: number; instructions: any }) => ({
          ...
          instructions: i.instructions.map((instruction: any) => ({
            ...
            data: base58.encode(Buffer.from(instruction.data || "", "base64")),
          })),
        })) || [],
    };
  }

  public parseTransactionAccounts<T extends Message | VersionedMessage>(message: T, loadedAddresses: T extends VersionedMessage ? LoadedAddresses | undefined : undefined = undefined): AccountMeta[] {
  ...
  private compiledInstructionToInstruction<Ix extends CompiledInstruction | MessageCompiledInstruction>(compiledInstruction: Ix, parsedAccounts: AccountMeta[]): TransactionInstruction {
  ...
  public flattenTransactionResponse(transaction: VersionedTransactionResponse): TransactionInstruction[] {
